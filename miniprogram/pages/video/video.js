...
Page({
  ...
  async getVideoGroupListData(){
    let {data:videoGroupListData} = await request('/video/group/list');
  ...
  async getVideoList(navId){
    ...
    let {data:videoListData} = await request('/video/group', {id: navId});
    
    // 获取视频数据
    // let videourl=await request('/video/url',{id:videoListData.datas.vid})
    ...
    let videoList = videoListData.datas.map(async item => {
      item.id = index++;
      let {data:vul}=await request('/video/url',{id:item.data.vid})
      item.playurl=vul.urls[0]
      return item;
    })
    videoList.forEach(async item => {
      ...
      console.log(vl);
      this.setData({
        videoList:this.data.videoList.concat(vl)
      })
    });
